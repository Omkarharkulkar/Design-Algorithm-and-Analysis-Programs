...
int main()
{
	...
	while (curw >= 0)
    {
        ...
        if (curw >= 0)
        {
			...
			cout << " completely in the bag, Space left: " << curw;
        }
        else
        {
            cout << "\nAdded object " << maxi + 1 << " Weight: "<< (array[0][maxi] + curw);
			cout << " Profit: "<< (array[1][maxi] / array[0][maxi]) * (array[0][maxi]+ curw);
			cout << " partially in the bag, Space left: 0"<< " Weight added is: " << curw + array[0][maxi];
